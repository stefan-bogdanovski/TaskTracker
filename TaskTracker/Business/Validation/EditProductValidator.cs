...
{
	public class EditProductValidator : AbstractValidator<ProjectDto>
	{
		private readonly TaskTrackerContext _context;
		public EditProductValidator(TaskTrackerContext context, int id)
		{
			_context = context;
            RuleFor(p => p.Name)
             .NotEmpty()
             .WithMessage("Name is a required parameter.")
             .Must((name) => !_context.Projects.Any(p => p.Name == name && p.Id != id))
